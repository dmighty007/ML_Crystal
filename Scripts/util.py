...
# Function to create a colored line based on time (index)
def color_plot(x, y, c = None, cmap='plasma', linewidth=2, ax = None, colorbar = False, cbar_label = "Time", fig = None):
    if ax is None:
        fig, ax = plt.subplots()
    if c is None:
        c = np.arange(len(x))
