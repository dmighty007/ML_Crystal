...
class TVAEEncoder(MLP):
    ...
    def __init__(self, units: List[int], nonlinearity=None, initial_batchnorm = True):
        import torch.nn as nn
        if nonlinearity is None:
            nonlinearity = nn.ELU
        super().__init__(units[:-1], nonlinearity=nonlinearity, initial_batchnorm=initial_batchnorm,
...
class TVAE(TAE):
    def __init__(self, encoder, decoder, optimizer='Adam',
                 learning_rate=5e-4, beta=1., device="cuda"):
